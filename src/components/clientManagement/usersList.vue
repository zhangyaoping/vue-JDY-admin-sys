  ...
  <el-row class="warp">
    <el-col :span="24" class="warp-breadcrum">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>商户提现审核</el-breadcrumb-item>
      </el-breadcrumb>
    </el-col>
    ...
    <el-col :span="24" class="warp-main">
      ...
      <!--列表-->
      <el-table :data="usersList" border highlight-current-row element-loading-text="拼命加载中" v-loading="listLoading"
                style="width: 100%;">
        ...
        <el-table-column prop="tradeAmount" label="提现金额" width="80"></el-table-column>
        <el-table-column prop="accountAmount" label="可用余额" width="80"></el-table-column>
        <el-table-column prop="sysUserName" label="审核人员" width="80"></el-table-column>
      ...
      <el-col :span="24" class="toolbar pad-a">
        <!--分页-->
        <div v-show="usersList.length" style="float: right" class="pagination-container">
          <el-pagination
            background
  ...
  export default{
    ...
    methods: {
      ...
      queryCostDetail() {
        ...
        Api.testApi.queryCashOutList(util.checkBe(para)).then(res => {
          if (res.code == '0000') {
            if (res.data.orderShopEvs) {
              for (let i = 0; i < res.data.orderShopEvs.length; i++) {
                res.data.orderShopEvs[i].createTime = util.formatDate.format(new Date(res.data.orderShopEvs[i].createTime), 'yyyy-MM-dd hh:mm:ss')
                res.data.orderShopEvs[i].type = this.returnTypeValue(res.data.orderShopEvs[i].type)
              }
              this.total = res.data.total;
              this.usersList = res.data.orderShopEvs;
